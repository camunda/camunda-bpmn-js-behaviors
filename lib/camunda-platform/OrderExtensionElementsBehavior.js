...
export default class OrderExtensionElementsBehavior extends CommandInterceptor {
  constructor(eventBus, modeling) {
    ...
    this.postExecuted('element.updateModdleProperties', LOW_PRIORITY, (context) => {
      const {
        element,
        moddleElement
      } = context;
      ...
      if (is(element, 'bpmn:CallActivity')) {
        this.orderCallActivityExtensionElements(element, moddleElement);
      }
    }, true);
  }

  orderCallActivityExtensionElements(element, moddleElement) {
    ...
    if (isEqual(moddleElement.get('values'), values)) {
      return;
    }
